...
def save_progress(races: Racecards, date: str, processed_races: list[tuple[str, str]], total_races: list[tuple[str, str]]) -> None:
    """Save current progress to a temporary file for resume capability."""
    progress_data = {
        'races': races,
        'date': date,
    ...
    temp_file = f'../racecards/{date}.temp'
    with open(temp_file, 'w', encoding='utf-8') as f:
        json.dump(progress_data, f, indent=2, default=str)

def load_progress(date: str) -> tuple[Racecards, list[tuple[str, str]], list[tuple[str, str]]] | None:
    ...
    try:
        ...
        races = data['races']
        processed_races = [tuple(r) for r in data['processed_races']]
        total_races = [tuple(r) for r in data['total_races']]

        print(f"Resuming from {len(processed_races)}/{len(total_races)} races processed")
        return races, processed_races, total_races
    except (json.JSONDecodeError, KeyError, FileNotFoundError):
        return None

def cleanup_temp_file(date: str) -> None:
...
def scrape_racecards(race_urls: dict[str, list[tuple[str, str]]], date: str) -> Racecards:
    ...
    existing_progress = load_progress(date)
    if existing_progress:
        races, processed_races, total_races = existing_progress
        remaining_races = [race for race in race_urls[date] if race not in processed_races]
    else:
        races: Racecards = defaultdict(lambda: defaultdict(lambda: defaultdict(dict)))
        processed_races = []
        remaining_races = race_urls[date]
        total_races = race_urls[date]

    if not remaining_races:
        print(f"All races already processed for {date}")
        cleanup_temp_file(date)
        return races
    ...
    for race_id, href in tqdm(
        ...
        ncols=91,
        initial=len(processed_races),
        total=len(total_races)
    ):
        ...
        try:
            ...
            if any(status != 200 for status in (status_racecard, status_runners, status_accordion)):
                ...
                processed_races.append((race_id, href))
                save_progress(races, date, processed_races, total_races)
                continue
            ...
            print(f'Unexpected error processing race {race_id}: {e}')
            # Save progress even on unexpected errors
            if (race_id, href) not in processed_races:
                processed_races.append((race_id, href))
            save_progress(races, date, processed_races, total_races)
            continue

    # Clean up temp file if all races processed successfully
    if len(processed_races) == len(total_races):
        cleanup_temp_file(date)
    else:
        # Save final progress if incomplete
        save_progress(races, date, processed_races, total_races)
...
def main() -> None:
    ...
    for date in race_urls:
        ...
        except Persistent406Error as e:
            print(f'Persistent 406 error encountered: {e}')
            print(f'Partial data saved to temp file for {date}')
            print('You can resume by running the same command again')
            continue
        except KeyboardInterrupt:
            print(f'Interrupted by user. Progress saved to temp file for {date}')
            print('You can resume by running the same command again')
            break
        except Exception as e:
            print(f'Unexpected error for {date}: {e}')
            print(f'Progress saved to temp file for {date}')
            continue
